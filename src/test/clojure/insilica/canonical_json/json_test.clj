...
(deftest read-numbers
  (is (= 42M (json/read-str "42")))
  (is (= -3M (json/read-str "-3")))
...
(deftest read-numbers-with-extra-input
  (is (= 123M (json/read-str "123 ")))
  (is (= 123M (json/read-str "123 a")))
  (is (= 123M (json/read-str "123 " :throw-on-extra-input? true)))
  (is (thrown-with-msg? clojure.lang.ExceptionInfo
        #"JSON error \(found extra"
        (json/read-str "123 4" :throw-on-extra-input? true))))

(deftest read-big-int
  (is (= 123456789012345678901234567890M
...
(deftest read-strings-with-extra-input
  (is (= "x" (json/read-str "\"x\" ")))
  (is (= "x" (json/read-str "\"x\"a")))
  (is (= "x" (json/read-str "\"x\" " :throw-on-extra-input? true)))
  (is (thrown-with-msg? clojure.lang.ExceptionInfo
        #"JSON error \(found extra"
        (json/read-str "\"x\" 4" :throw-on-extra-input? true))))
...
(deftest read-arrays-with-extra-input
  (is (= [] (json/read-str "[] ")))
  (is (= [] (json/read-str "[]a")))
  (is (= [] (json/read-str "[] " :throw-on-extra-input? true)))
  (is (thrown-with-msg? clojure.lang.ExceptionInfo
        #"JSON error \(found extra"
        (json/read-str "[] []" :throw-on-extra-input? true))))

(deftest read-objects
  (is (= {:k1 1M, :k2 2M, :k3 3M, :k4 4M, :k5 5M, :k6 6M, :k7 7M, :k8 8M
          :k9 9M, :k10 10M, :k11 11M, :k12 12M, :k13 13M, :k14 14M, :k15 15M, :k16 16M} 
...
(deftest read-objects-with-extra-input
  (is (= {} (json/read-str "{} ")))
  (is (= {} (json/read-str "{}a")))
  (is (= {} (json/read-str "{} " :throw-on-extra-input? true)))
  (is (thrown-with-msg? clojure.lang.ExceptionInfo
...
(deftest convert-values
  (is (= {:number 42M :date (java.sql.Date. 55 6 12)}
...
(deftest pass1-test
  (let [input (json/read-str pass1-string)]
    ...
    (is (= 1234567890M (get-in input [8 "integer"])))
